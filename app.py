...
# --- CSS STYLING (Remains the same for consistency) ---
st.markdown("""
<style>
...
    st.error(f"❌ Error initializing Groq client. Details: {e}")
    st.stop()
...
                st.session_state.weak_topics = weak_topics # Update session state
                is_data_corrupted = any(len(t) > 50 for t in weak_topics)
                
                # Determine current quiz state
                if is_data_corrupted:
                    st.error("🚨 **CRITICAL DATA ERROR:** Your progress tracker contains corrupted topic names. The Focus Quiz cannot be generated until this is fixed.")
...
                    total = stats['total']
                    correct = stats['correct']
                    percentage = (correct / total) * 100 if total > 0 else 100.0
                    
                    if len(concept) > 50:
